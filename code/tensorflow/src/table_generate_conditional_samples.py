...
import tqdm
from load_dataset import load_tables
import argparse
...
parser.add_argument('--data_dir', type=str, default='./afs/', help='Path to models directory')
...
parser.add_argument('--header_num', type=int, default=3, help='Pretrained model name')
...
def inference_model(
    ...
    chunks_tables = load_tables(enc, table_path)[0]
    ...
    for ckpt_item in ckpt_list:
        generation = []
        with tf.Session(graph=tf.Graph()) as sess:
            mha1 = tf.keras.layers.MultiHeadAttention(num_heads=header_num, key_dim=256)
            mha2 = tf.keras.layers.MultiHeadAttention(num_heads=header_num, key_dim=256)
            mha15 = tf.keras.layers.MultiHeadAttention(num_heads=header_num, key_dim=256)
            tables = tf.placeholder(tf.int32, [batch_size, 56, 24, 122])
            context = tf.placeholder(tf.int32, [batch_size, None])
            np.random.seed(seed)
            tf.set_random_seed(seed)
            output = sample.table_sample_sequence_beam_search(
                hparams=hparams, length=length,
                context=context,
                tables=tables,
                beam_width=beam_width,
                keras_layers=[mha1, mha2, mha15],
                #keras_layers=[mha1, mha2],
                temperature=temperature, top_k=top_k, top_p=top_p
            )
            saver = tf.train.Saver()
            ...
            saver.restore(sess, ckpt)
            generated = 0
            for index, raw_text in enumerate(context_data):
                context_tokens = enc.encode(raw_text)
                generation_item = []
                for _ in range(nsamples // batch_size):
                    '''
                    ...
                    out1, out2 = sess.run(output, feed_dict={
                        context: [context_tokens for _ in range(batch_size)],
                        tables: [chunks_tables[index] for _ in range(batch_size)]
                    })
...
if __name__ == '__main__':
    ...
    if 'train' in args.data_dir:
        path = 'afs/data/table_train.json'
    elif 'val' in args.data_dir:
        path = 'afs/data/table_val.json'
    elif 'test' in args.data_dir:
        path = 'afs/data/table_test.json'
    else:
        print(args.data_dir)
        exit(1)
    if args.model == 'normal':
        ...
        #inference_model('-1', ckpt_dir='afs/checkpoint/'+args.ckpt_dir, ckpt_list=["1"], model_name=args.model_name,
            data_dir='afs/data/'+args.data_dir, out_dir='afs/data/'+args.out_dir, table_path=path, header_num=args.header_num)
        ...
        data_dir='afs/rewrite/'+args.data_dir, out_dir='afs/rewrite/'+args.out_dir, table_path=path, header_num=args.header_num)

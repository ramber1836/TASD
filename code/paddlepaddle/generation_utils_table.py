...
class GenerationMixin(object):
    ...
    @staticmethod
    def expand_table_inputs_for_generation(table_ids,
                                     expand_size):

        index = paddle.tile(
            paddle.arange(paddle.shape(table_ids)[0]).unsqueeze(-1),
            [1, expand_size]).reshape([-1])

        table_ids = paddle.gather(table_ids, index)
    ...
    def prepare_inputs_for_generation(self, input_ids, table_ids, **kwargs):
        # Implement in subclasses for custom behavior to prepare inputs in the
        # generate method.

        return {"input_ids": input_ids, "table_ids": table_ids}
    ...
    def generate(self,
        ...
        if decode_strategy == 'greedy_search':
            ...
            return self.greedy_search(input_ids, table_ids, logits_processors, max_length,
                                      pad_token_id, eos_token_id,
                                      **model_kwargs)
            ...
            return self.sample(input_ids, table_ids, logits_processors, max_length,
            ...
            if num_beam_groups > 1:
                ...
                table_ids = self.expand_table_inputs_for_generation(table_ids, expand_size=num_beams)

                return self.beam_search(
                    input_ids, table_ids, beam_scorer, logits_processors, max_length,
                    diversity_rate, pad_token_id, eos_token_id, **model_kwargs)

    def greedy_search(self, input_ids, table_ids, logits_processors, max_length,
        ...
        while cur_len < max_length:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, table_ids,
                                                              **model_kwargs)
    ...
    def beam_search(self, input_ids, table_ids, beam_scorer, logits_processors, max_length,
                    diversity_rate, pad_token_id, eos_token_id, **model_kwargs):
    ...
    def group_beam_search(self, input_ids, table_ids, beam_scorer, logits_processors,

...
class GPTModel(GPTPretrainedModel):
    ...
    def __init__(self,
        ...
        #extra parts start
        self.n_stx=n_stx
        self.n_ttx_row=n_ttx_row
        self.n_ttx_col=n_ttx_col
        self.num_head=num_head

        self.position_embeddings_twper = nn.Embedding(
            self.n_ttx_row,
            hidden_size,
            weight_attr=paddle.ParamAttr(
                name="pos_embeddings_extra_twper",
                initializer=nn.initializer.Normal(
                    mean=0.0, std=initializer_range)))
        
        self.position_embeddings_twpec = nn.Embedding(
            self.n_ttx_col,
            hidden_size,
            weight_attr=paddle.ParamAttr(
                name="pos_embeddings_extra_twpec",
                initializer=nn.initializer.Normal(
                    mean=0.0, std=initializer_range)))

        self.position_embeddings_swpe = nn.Embedding(
            self.n_stx,
            hidden_size,
            weight_attr=paddle.ParamAttr(
                name="pos_embeddings_extra_swpe",
                initializer=nn.initializer.Normal(
                    mean=0.0, std=initializer_range)))

        self.mha_sentence = MHA(embed_dim=hidden_size, num_heads=self.num_head)
        self.mha_table = MHA(embed_dim=hidden_size, num_heads=self.num_head)
        self.mha_cross = MHA(embed_dim=hidden_size, num_heads=self.num_head)
        self.norm_sentence = nn.LayerNorm(hidden_size, epsilon=1e-5)
        self.norm_table = nn.LayerNorm(hidden_size, epsilon=1e-5)
        self.norm_cross = nn.LayerNorm(hidden_size, epsilon=1e-5)
        self.norm_merge = nn.LayerNorm(hidden_size, epsilon=1e-5)
        self.seq_attn_mask = paddle.rand((1344, 3, 122, 122))
        self.attn_mask = paddle.rand((1, 3, 1344, 1344))
        self.cross_attn_mask = paddle.rand((1, 3, 1024, 1344))
        #extra parts finish
    ...
    def forward(self,
        ...
        #extra start
        batch, row, col, seq_len = table_ids.shape
        table_embedding = self.embeddings.word_embeddings(table_ids) + self.position_embeddings_swpe(paddle.tile(paddle.reshape(paddle.arange(self.n_stx), [1, 1, 1, self.n_stx]), [batch, self.n_ttx_row, self.n_ttx_col, 1])) # [1,56,24,122,768]
        table_position_embedding = self.position_embeddings_twper(paddle.tile(paddle.reshape(paddle.arange(self.n_ttx_row), [1, self.n_ttx_row, 1]), [batch, 1, self.n_ttx_col])) \
                        + self.position_embeddings_twpec(paddle.tile(paddle.reshape(paddle.arange(self.n_ttx_col), [1, 1, self.n_ttx_col]), [batch, self.n_ttx_row, 1]))#[1,56,24,768]
        batch, row, col, seq_len, dim = table_embedding.shape
        
        attn_input = paddle.reshape(self.norm_sentence(table_embedding), [batch * row * col, seq_len, dim]) #[1344,122,768]
        
        #print(attn_input.shape)
        seq_self_attn = self.mha_sentence(attn_input, attn_input, attn_input)#[1344,122,768]
        seq_self_attn = paddle.reshape(seq_self_attn, [batch, row, col, seq_len, dim])#[1,56,24,122,768]
        seq_self_attn = paddle.mean(seq_self_attn, axis=-2) # [1,56,24,768]
        
        table_attn_input = paddle.reshape(self.norm_table(seq_self_attn + paddle.mean(table_embedding, -2)), [batch, row*col, dim]) #[1,1344,768]?为什么还要和原先的table_embedding加起来
        table_self_attn = self.mha_table(table_attn_input, table_attn_input, table_attn_input)#[1,1344,768]
        table_self_attn = paddle.reshape(table_self_attn, [batch, row, col, dim])#[1,56,24,768]
        
        cross_attn_input = paddle.reshape(self.norm_cross(table_self_attn + table_position_embedding), [batch, row*col, dim])#[1,1344,768]
        #print(encoder_outputs.shape, cross_attn_input.shape)
        cross_attn = self.mha_cross(encoder_outputs, cross_attn_input, cross_attn_input)#[1,1024,768]

        encoder_outputs = self.norm_merge(encoder_outputs + cross_attn) #[1,1024,768]
        #extra finish
